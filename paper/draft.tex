...
My headline finding is a contrast between the full-sample and a conflict-active subsample. In the full panel, instrumented fire intensity (log FRP) has essentially zero effect on conflict counts, a null that survives a Poisson control-function specification designed for severely zero-inflated count data \citep[following][]{lu2023fish}. However, this null masks substantial heterogeneity: in districts where conflict is not a rare event (defined as districts with conflict activity in at least 30\% of sample months), a one-log-point increase in local fire intensity reduces conflict events by 0.11 events per district-month ($p=0.005$) and political-violence events by 0.046 ($p=0.006$). Splitting this restricted-sample effect by a composite ACLED event category shows the effect operates through riots and protests ($-0.046$, $p=0.002$), with no detectable effect on battles or organized violence ($p=0.964$). This initially suggests a \emph{disruption} mechanism, in which severe haze deters the outdoor gatherings that riots and protests require while leaving organized armed confrontation unaffected. A finer decomposition into literal ACLED event types complicates this clean interpretation, however (Section~\ref{sec:fourway}): the effect is concentrated in riots and \emph{violence against civilians} rather than in peaceful protests, so the mechanism appears narrower, and less well understood, than a simple disruption of planned public assembly. I develop this qualification in Sections~\ref{sec:fourway} and~\ref{sec:discussion}.

The remainder of the paper proceeds as follows. Section~\ref{sec:litreview} relates this paper to three literatures: climate and conflict, wind- and pollution-instrumented designs linking air quality to behavior, and Indonesia-specific haze research. Section~\ref{sec:data} describes the data and panel construction. Section~\ref{sec:preliminary} reports preliminary, non-instrumented correlational evidence and motivates the IV design. Section~\ref{sec:empirical} describes the instrument and empirical strategy. Section~\ref{sec:results} presents results, including the full-sample null, the conflict-active heterogeneity analysis, a two-category and a finer four-category event-type decomposition, the Poisson robustness check, the fatalities analysis, a placebo test, shared-seasonality check, and Conley spatial-correlation standard errors for the exclusion restriction and inference, and a fully-instrumented distributed-lag specification. Section~\ref{sec:discussion} discusses interpretation and limitations, Section~\ref{sec:future} outlines future research directions, and Section~\ref{sec:conclusion} concludes.
...
Figures~\ref{fig:conflictmap} and~\ref{fig:firemap} map the raw conflict and fire data underlying the panel; the conflict map shows \emph{all} recorded events in the sample window, of which the 98.6\% matched to a district enter the analysis. Conflict events are concentrated in Java, Sumatra, and Papua, while fire activity is concentrated in the peatland and forest regions of Sumatra, Kalimantan, and Papua -- areas that are spatially distinct from, but geographically adjacent to, the most conflict-active districts, consistent with the smoke-transport mechanism the upwind instrument is designed to exploit.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/conflict_events_map.png}
\caption{\emph{All} ACLED conflict events recorded in Indonesia, February 2015--June 2025 ($n=21{,}653$). The map shows every recorded event; the district-month panel uses the 98.6\% (21{,}393) \emph{matched} to a GADM Level-2 district (Section~\ref{sec:data}).}
...
The negative effect identified in Table~\ref{tab:active} is concentrated entirely in riots and protests: a one-log-point increase in fire intensity reduces riot/protest events by 0.046 ($p=0.002$), while the effect on battles and organized violence is a precisely-estimated zero ($0.0003$, $p=0.964$). This pattern is informative about mechanism: riots and protests are, almost by definition, public, outdoor, assembly-based forms of contention, while battles and remote violence are typically planned and organized by armed actors whose operations are less sensitive to ambient air quality. The code implementing this decomposition can be found at \href{https://github.com/Hariaksha/climate-conflict/blob/main/analysis/wind-IV.ipynb}{this link}. The \emph{disruption-of-assembly} interpretation suggested by this two-category split, however, is substantially complicated by the finer decomposition in the next subsection.

\subsection{A finer decomposition: literal ACLED categories}
\label{sec:fourway}

The \emph{Riots/Protests} category in Table~\ref{tab:eventtype} is a composite of two ACLED event types, \emph{Riots} and \emph{Violence against civilians}; it does not include ACLED's literal \emph{Protests} category, the single largest event type in the matched data (80.7\% of all matched events) and the one most directly tied to planned, public assembly. If the disruption-of-assembly mechanism proposed above is correct, this category should show the clearest negative effect of all. This section re-estimates the IV specification separately for four literal, non-overlapping ACLED categories: \emph{Protests}, \emph{Riots}, \emph{Violence against civilians}, and \emph{Strategic developments} (the last included as an additional placebo-style category, since strategic developments, e.g. arrests, recruitment, and looting, do not require public assembly and were not expected to respond under the original mechanism). Table~\ref{tab:fourway} reports both the partially-instrumented specification (matching Table~\ref{tab:eventtype}) and the fully-instrumented specification of Section~\ref{sec:fulliv}.

\begin{table}[H]
\centering
\caption{Four-way event-type decomposition, literal ACLED categories, $\tau \geq 30\%$ sample}
\label{tab:fourway}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
 & Protests & Riots & Violence against civilians & Strategic developments \\
\midrule
\multicolumn{5}{l}{\emph{Partially instrumented}} \\
$\text{logFRP}_{dt}$ & $-0.0544$ [$p=0.266$] & $-0.0231^{*}$ [$p=0.067$] & $-0.0231^{**}$ [$p=0.021$] & $-0.0114^{*}$ [$p=0.075$] \\
\midrule
\multicolumn{5}{l}{\emph{Fully instrumented}} \\
$\text{logFRP}_{dt}$ & $-0.0463$ [$p=0.302$] & $-0.0318^{***}$ [$p=0.007$] & $-0.0336^{***}$ [$p=0.003$] & $-0.0071^{*}$ [$p=0.085$] \\
$\text{logFRP}_{d,t-1}$ & $-0.0172$ [$p=0.708$] & $0.0256^{**}$ [$p=0.031$] & $0.0310^{***}$ [$p=0.001$] & $-0.0051$ [$p=0.416$] \\
\bottomrule
\end{tabular}%
}
\begin{minipage}{0.95\textwidth}
\vspace{4pt}
\footnotesize \emph{Notes:} Each column is a literal, non-overlapping ACLED event type (verified directly against the raw event-type field), not a composite category. ``Partially instrumented'' matches the specification in Table~\ref{tab:eventtype}; ``fully instrumented'' matches Section~\ref{sec:fulliv}. Standard errors clustered by province. $N=10{,}750$ for all columns. $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.1$.
\end{minipage}
\end{table}

This decomposition complicates, rather than confirms, the disruption-of-assembly interpretation. \emph{Protests}, the category most directly tied to planned public assembly and the largest by far, shows no statistically significant effect in either specification ($p=0.27$--$0.30$). The negative effect previously attributed to the ``Riots/Protests'' composite is instead driven by \emph{Riots} and, more surprisingly, \emph{Violence against civilians}, a category that does not obviously require public assembly in the way a riot or protest does; both are significant at the 1\% level under full instrumentation and both display the same positive, significant one-month-lagged rebound documented in Section~\ref{sec:fulliv}. \emph{Strategic developments}, included as a placebo-style category that should show no effect under either the original or the revised mechanism, instead shows a marginally significant negative effect ($p=0.075$--$0.085$), weaker than Riots or Violence against civilians but not the precise null that \emph{Battles/Violence} shows in Table~\ref{tab:eventtype}.

Taken together, these results do not support a mechanism specifically about disruption of \emph{planned} public assembly, since Protests, the clearest example of such an event, is unaffected. They are more consistent with a narrower mechanism concentrated in spontaneous, often violent crowd events (Riots) and direct violence against civilians, though the weak effect on Strategic developments is not fully explained by this narrower story either, and could partly reflect reduced observability or reporting of lower-salience events during severe haze episodes rather than a purely behavioral channel. I do not have direct evidence to distinguish between these explanations with the data at hand, and I revise the interpretation in Section~\ref{sec:discussion} accordingly: the mechanism underlying the conflict-active heterogeneity result is real but less precisely a story about ``disruption of public assembly'' than the two-category decomposition in Table~\ref{tab:eventtype} alone would suggest.
...
\begin{enumerate}
    \item \textbf{Stage 1:} Regress $\text{logFRP}_{dt}$ on the excluded instruments $(\text{logZ}_{dt}, \text{logZ}_{d,t-1})$ \emph{and} the exogenous lag control $\text{logFRP}_{d,t-1}$, with district and year-month fixed effects, and compute the residual $\hat{v}_{dt} = \text{logFRP}_{dt} - \widehat{\text{logFRP}}_{dt}$. Including the exogenous lag control in the first stage (rather than only the excluded instruments) ensures $\hat{v}_{dt}$ is orthogonal to $\text{logFRP}_{d,t-1}$, as a correctly-specified two-stage residual inclusion estimator requires.
...
\toprule
 & events & pv\_events \\
\midrule
$\text{logFRP}_{dt}$       & $-0.0297$ & $-0.0133$ \\
                            & (0.0183) & (0.0434) \\
                            & [$p=0.103$] & [$p=0.759$] \\
$\text{logFRP}_{d,t-1}$    & $0.0045$ & $0.0064$ \\
                            & (0.0100) & (0.0231) \\
                            & [$p=0.652$] & [$p=0.780$] \\
$\hat{v}_{dt}$ (control function) & $0.0219$ & $0.0397$ \\
                            & (0.0187) & (0.0446) \\
                            & [$p=0.242$] & [$p=0.374$] \\
\midrule
District FE     & Yes & Yes \\
...
\begin{minipage}{0.85\textwidth}
\vspace{4pt}
\footnotesize \emph{Notes:} Stage 1 regresses $\text{logFRP}_{dt}$ on the excluded instruments and the exogenous lag control $\text{logFRP}_{d,t-1}$ (with district and year-month fixed effects); $\hat{v}_{dt}$ is the resulting residual. Stage 2 is a Poisson fixed-effects regression. $^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.1$.
\end{minipage}
\end{table}

In contrast to \citet{lu2023fish}, the control-function residual $\hat{v}_{dt}$ is statistically insignificant in both the \texttt{events} ($p=0.242$) and \texttt{pv\_events} ($p=0.374$) Poisson models, indicating that endogeneity bias in the full-sample relationship between local fire intensity and conflict is minimal. The Poisson coefficient on $\text{logFRP}_{dt}$ itself also remains statistically insignificant in both outcomes. I therefore conclude that the full-sample null is robust to (i) moving from a linear to a count-data (Poisson) specification, and (ii) explicitly controlling for first-stage endogeneity via the control-function approach. This strengthens the interpretation of the conflict-active heterogeneity in Section~\ref{sec:active} as a genuine feature of the data, a \emph{local} average treatment effect concentrated among districts where conflict is a meaningful, recurring outcome, rather than an artifact of model misspecification in the full sample.
...
Poisson IV control function (full panel) & $-0.8832^{***}$ (0.0599) [$p<0.001$] & $+0.7868^{***}$ (0.0613) [$p<0.001$] & 20{,}496 \\
\bottomrule
\end{tabular}%
}
\begin{minipage}{0.95\textwidth}
...
Six results emerge from this analysis. First, the upwind-fire instrument is extremely strong (first-stage $F \approx 4{,}447$), confirming that the design has ample power to detect effects of fire-driven exposure on local fire intensity and, through it, on downstream outcomes, if such effects exist. Second, the \emph{average} effect of fire intensity on all conflict events across the full population of 447 districts is null under the baseline specification, and this null is robust across linear IV and Poisson IV control-function specifications. Third, this average null masks heterogeneity: among the subset of districts where conflict is not a rare event, fire intensity has a sizeable, negative effect on conflict and on political violence specifically. Fourth, the strength of this evidence varies by outcome once spatial correlation in the errors is accounted for: the riots/protests result is robust to Conley standard errors and to an added province $\times$ calendar-month fixed effect (Section~\ref{sec:robustness}), while the broader all-events and political-violence-event results in Table~\ref{tab:active} are not -- they fall short of conventional significance under Conley standard errors, and the all-events result loses significance under the added seasonal fixed effect. Fifth, fully instrumenting the distributed-lag specification (Section~\ref{sec:fulliv}) confirms the conflict-active heterogeneity result is not an artifact of the single-endogenous-regressor simplification used elsewhere in the paper, but also reveals that the full-panel null is less clean than the baseline specification suggests: properly instrumenting the lag uncovers a significant negative contemporaneous effect on political-violence events in the full panel, alongside a new, economically large, positive lagged effect that is suggestive of a short-run disruption followed by a partial rebound. Sixth, and importantly, the finer event-type decomposition in Section~\ref{sec:fourway} complicates the mechanism: the effect is \emph{not} concentrated in Protests, the literal ACLED category most directly tied to planned public assembly, but instead in Riots and, unexpectedly, Violence against civilians, alongside a weaker effect on Strategic developments that a clean disruption-of-assembly story does not predict. I therefore treat the riots/protests-specific finding from the two-category decomposition as suggestive of a real but narrower and less well-understood mechanism than originally proposed, the broader conflict-active heterogeneity in Table~\ref{tab:active} as informative but less conclusive evidence of the same underlying pattern, and the full-panel null as itself only a partial null that does not extend cleanly to political-violence events once the lag structure is fully instrumented.

\paragraph{Interpretation.} The pattern documented here is more nuanced than a clean disruption-of-collective-action story. The original two-category decomposition (Table~\ref{tab:eventtype}) is consistent with haze suppressing the public gatherings that riots and protests require, without affecting organized armed violence. However, the finer decomposition in Section~\ref{sec:fourway} shows that \emph{Protests}, the event type most directly defined by planned public assembly, does not respond, while \emph{Violence against civilians}, which does not require assembly in the same sense, responds as strongly as \emph{Riots}. This pattern is more consistent with a mechanism concentrated in spontaneous, often violent crowd events specifically, rather than disruption of organized public assembly in general, though I cannot fully rule out that part of the pattern, particularly the weaker effect on \emph{Strategic developments}, reflects reduced observability or reporting of lower-salience events during severe haze rather than a purely behavioral channel. Whatever its precise channel, the direction of the effect is the opposite sign from much of the climate-conflict literature, which typically finds that adverse environmental shocks \emph{increase} conflict through resource scarcity or income-shock channels \citep{hsiang2013quantifying,burkehsiangmiguel2015,miguelsatyanathsergenti2004}. The contrast may reflect the very different time horizon at play: fire-driven haze is an acute, short-duration shock operating through immediate behavioral and logistical channels (people cannot or do not want to go outside), rather than a persistent shock to income or resource availability that might generate grievance-based conflict over a longer horizon.
...
\paragraph{Caveats and next steps.} Several issues merit further attention in the next draft. First, the conflict-active sample restriction raises an external-validity question: the documented effect applies to the subpopulation of districts where conflict is already a recurring phenomenon, and Section~\ref{sec:active} should be explicitly framed as a heterogeneity/local-effect analysis rather than a population-average effect, with the full-sample null in Table~\ref{tab:mainiv} as the headline specification, though Section~\ref{sec:fulliv} shows that null is itself not fully robust for \texttt{pv\_events}. Second, the placebo test in Section~\ref{sec:robustness} supports the exclusion restriction, but the shared-seasonality and Conley standard-error checks in the same section both show that the all-events and political-violence results in Table~\ref{tab:active} are less robust than the baseline specification suggests: \texttt{events} loses significance under the added province $\times$ calendar-month fixed effect, and both \texttt{events} and \texttt{pv\_events} fall short of conventional significance under Conley standard errors. The riots/protests result in Table~\ref{tab:eventtype}, which had carried the paper's central mechanism claim, is robust to both checks, and to fully instrumenting the distributed lag (Section~\ref{sec:fulliv}), but Section~\ref{sec:fourway} shows that result itself is not well explained by a clean disruption-of-planned-assembly story: Protests, the literal category most directly tied to that mechanism, is unaffected, while Riots and Violence against civilians drive the effect instead. This sensitivity should be reported alongside the headline estimates rather than omitted, and the next draft should lead with the four-category decomposition as the most informative and most honest specific finding, with the broader event-count and two-category results presented as suggestive of a real but still poorly-understood mechanism rather than conclusive evidence of disruption-of-assembly specifically. Third, the positive and often significant lagged coefficient uncovered in Section~\ref{sec:fulliv} once the lag is properly instrumented was not anticipated by the original design and warrants dedicated investigation rather than the brief treatment given here. Fourth, the marginally significant effect on Strategic developments (Section~\ref{sec:fourway}), a category not predicted to respond under any version of the mechanism considered here, raises the possibility that part of the documented pattern reflects reduced reporting or observability of conflict during severe haze rather than a purely behavioral channel; distinguishing these explanations would require external data on media coverage or ACLED source density during high- versus low-haze periods, which is beyond the scope of this draft.
...
\paragraph{Distinguishing a spontaneous-violence channel from a reporting channel.} Section~\ref{sec:fourway} finds that the negative effect is concentrated in Riots and Violence against civilians, not in the literal Protests category, and that Strategic developments shows a weaker but still marginally significant effect. This pattern is consistent with at least two distinct explanations that the current data cannot separate: (i) haze specifically disrupts spontaneous, often violent crowd events while leaving planned assembly unaffected, or (ii) haze reduces the observability or media/ACLED-source reporting of lower-salience events generally, mechanically lowering recorded counts across several event types regardless of behavioral channel. Distinguishing these would require external data on local media coverage, ACLED source density, or an independent measure of ground-truth event occurrence (e.g. satellite-detected nighttime gathering activity) during high- versus low-haze periods, none of which are used in this draft.

\paragraph{Mechanism validation with finer-grained data.} The disruption interpretation in Section~\ref{sec:eventtype} and Section~\ref{sec:fourway} is inferred from the contrast across event-type categories, but is not directly tested. Daily (rather than monthly) conflict and pollution data, for example ground-level PM2.5 or satellite aerosol optical depth merged with daily ACLED event timing, would allow a sharper test of whether riot activity drops specifically on high-haze days, and whether any displaced activity reappears once air quality improves (a substitution rather than elimination effect).
...
Using a novel district-month panel for Indonesia and an instrument based on the physical transport of wildfire smoke, I find no average effect of fire-driven pollution exposure on overall conflict under the baseline specification, but a robust and economically meaningful negative effect among conflict-active districts. This negative effect is concentrated in riots and in violence against civilians rather than in peaceful protests, so it is better described as a suppression of spontaneous, crowd-based, and street-level contention than as a clean disruption of planned public assembly; the precise behavioral channel, and the possibility that it partly reflects reduced reporting of lower-salience events during severe haze, remains an open question. These results nonetheless suggest that short-run environmental shocks can affect conflict not only by altering incentives for violence, but by mechanically disrupting the day-to-day conditions under which low-level, street-level contention occurs.

This paper sits at the intersection of two literatures that, to date, have largely run in parallel: the economics literature on climate and conflict, which typically documents that adverse environmental shocks increase conflict through resource-scarcity or income channels \citep{hsiang2013quantifying,burkehsiangmiguel2015}, and a newer literature using wind- and pollution-based instruments to show that short-run air pollution increases individual aggressive and criminal behavior \citep{herrnstadt2021,bondyrothsager2020,burkhardt2019}. By documenting a negative effect on crowd-based, street-level forms of contention, this paper suggests that the disruption channel through which environmental shocks affect conflict can run in the opposite direction from the individual-behavior findings in that second literature, a distinction that existing work has not, to my knowledge, explicitly tested.
